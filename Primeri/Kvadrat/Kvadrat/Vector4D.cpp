...
Vector4D Vector4D::CrossProduct3D(Vector4D& A) {
	double i = y * A.z - z * A.y;
	double j = x * A.z - z * A.x;
	double k = x * A.y - y * A.x;
	return Vector4D(i, j, k, 0.0);
}
...
double Vector4D::AngleBetween(Vector4D& A){
	Vector4D B(x, y, z, t);
