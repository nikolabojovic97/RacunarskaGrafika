...
void Matrix4x4::LoadRotationMatrix(Vector4D& p, Vector4D& v, double alpha){
	Matrix4x4 tranP, tranPMinus, rotateFi, rotateFiMinus, rotateAlfa, rotateTeta, rotateTetaMinus;
	double teta, fi, d;
	...
	if(v.X() == 0) teta = 0;
		...
		if (v.Z() != 0) teta = atan(v.X() / v.Z());
			...
			if (v.X() > 0) teta = 3.1415926 / 2;
			else teta = -3.1415926 / 2;
		}
	}
	if(v.Y() == 0) fi = 0;
	else{
	...
	*this = tranP * rotateTeta * rotateFiMinus * rotateAlfa* rotateFi* rotateTetaMinus* tranPMinus;
}
